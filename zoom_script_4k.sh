...
" -map "[outv]" -map 0:a -c:v libx264 -c:a copy -y Boom_Test_scaled_then_zoomed_4k.mp4
step_end=$(date +%s)
echo "[INFO] ffmpeg 4K缩放处理完成，耗时 $((step_end-step_start)) 秒"
