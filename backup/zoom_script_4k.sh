...
if [ "$INTERPOLATE" = "true" ]; then
    echo "[INFO] 开始执行 ffmpeg 4K + ${FPS}帧 (minterpolate插帧) 缩放动画处理..."
    FPS_FILTER="minterpolate=fps=$FPS"
    OUTPUT_SUFFIX="_mint"
else
    echo "[INFO] 开始执行 ffmpeg 4K + ${FPS}帧 (普通fps) 缩放动画处理..."
    FPS_FILTER="fps=$FPS"
    OUTPUT_SUFFIX=""
fi

ffmpeg -hide_banner -i "Boom_Test.mp4" -filter_complex "
[0:v]${FPS_FILTER},scale=3840:2160,split=3[pre][zoom][post];
...
" -map "[outv]" -map 0:a -c:v libx264 -r $FPS -c:a copy -y Boom_Test_zoom_4k_${FPS}fps_to_original${OUTPUT_SUFFIX}.mp4

step_end=$(date +%s)
echo "[INFO] ffmpeg 4K + ${FPS}帧缩放动画处理完成，耗时 $((step_end-step_start)) 秒"
